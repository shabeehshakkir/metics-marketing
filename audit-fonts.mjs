...
for (const [wName, width, height] of widths) {
    ...
    for (const [name, path] of routes) {
        ...
        const fonts = await page.evaluate(() =>
            Array.from(document.fonts).map((f) => `${f.family} ${f.style} ${f.weight} ${f.status}`)
        );
        results.push({ route: name, width: wName, ...overflow, fonts: [...new Set(fonts)] });
        await page.screenshot({ path: `${OUT}/font-${name}-${wName}.png`, fullPage: true });
...
console.log('\nLoaded fonts (home desktop):');
console.log(results[0].fonts.join('\n'));
